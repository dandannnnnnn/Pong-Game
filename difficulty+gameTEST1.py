...
def on_message(client, userdata, msg):
    ...
    if payload in ["links", "rechts", "hold"]:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if player_name.strip():
                        ...
                        # Optional: feedback that name cannot be empty
                        pass
...
def display_highscores_screen(): # Renamed from user's display_highscores for clarity
    screen.fill(WHITE)
...
show_highscores_button = {"text": "Highscores", "rect": pygame.Rect(0, 0, button_width, button_height), "base_text_color": pastel_purple, "action": "show_highscores"} # "Highscores" text
...
while running: 
    ...
    if not in_difficulty_menu and not game_over and not showing_highscores_page:
        if mqtt_paddle_command == "links": 
            pallet_y -= pallet_speed
        elif mqtt_paddle_command == "rechts": 
            pallet_y += pallet_speed
        ...
        if ball_rect.colliderect(pallet_rect): 
            if speedBall_x > 0 and positionBall_x < pallet_x + pallet_width : 
